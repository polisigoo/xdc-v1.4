    ...
    <div class="k1_manage_table">
        ...
        <div v-if="!spinner_loading">
            ...
            <div v-else>
                ...
                <div class="table-responsive mt-2" v-if="Object.keys(data_search).length  > 0  && data_search.series !== null ">
                            ...
                            <tr v-for="(item, index) in data_search.series" :key="index">
                                ...
                                <td>
                                    <div class="btn-group" role="group">
                                        <router-link class="btn btn-sm btn-warning btn-sm mr-2" :to="{name:'analysis-series', params: {id:item.id}}" role="button">Analysis</router-link>
                                        <button class="btn btn-sm btn-warning btn-sm mr-2" @click="ADD_TO_TOP(item.id,index)" v-if="item.series_id !== item.id"
                                                :id="item.id">
                                                Top
                                            </button>
                                        <router-link class="btn btn-sm btn-warning btn-sm mr-2" :to="{name:'series_edit', params: {id:item.id}}" role="button">Edit
                                        </router-link>
                                        <router-link class="btn btn-sm btn-warning btn-sm mr-2" :to="{name:'series_manage_id', params: {id:item.id}}" role="button">Show
                                        </router-link>
                                        <button v-if="!button_loading" class="btn btn-sm btn-danger btn-sm mr-2" @click="DELETE(item.id,index)" :id="item.id">Delete</button>
                ...
                <div class="table-responsive mt-2" v-if="data_search.series === null || Object.keys(data_search).length === 0">
                    <div class="table table-hover">
                        <thead>
                            <th>Cover</th>
                            <th>Name</th>
                ...
                <nav aria-label="pagination" class="m-4 p-1" v-if="data_search.series === null || Object.keys(data_search).length === 0">
                    <ul class="pagination ">
