      ...
      <h5 class="title p-2">Theme Manage</h5>
        <div class="m-2" id="manage-panel">
            <ul class="nav nav-tabs">
                <li class="nav-item">
                    <router-link class="nav-link" :to="{name: 'themes'}">Themes</router-link>
                    
                </li>
                <li class="nav-item">
                    <router-link class="nav-link" :to="{name: 'footer'}">Footer</router-link>
                </li>
            </ul>
        </div>
      <hr>
        ...
        <div class="row">
          ...
          <div class="col-12 col-sm-4 col-lg-4 theme-box" v-for="(item,index) in data" :key="index">
            ...
            <div class="control mt-1">
              <div class="group-btn">
                <button v-if="!button_loading && item.status === 0" @click="SET(item.id)" class="btn btn-sm btn-warning">Set</button>
                <button v-else class="btn btn-sm btn-success" disabled>Active</button>
                <button v-if="button_loading === item.id" class="btn btn-sm btn-warning" disabled>
                  <i id="btn-progress"></i> Loading</button>

                <button v-if="!button_loading && item.name !== 'default' && item.status !== 1 " @click="DELETE(item.id,index)" class="btn btn-sm btn-danger">Delete</button>
                <button v-if="delete_loading === item.id" class="btn btn-sm btn-danger" disabled>
                  <i id="btn-progress"></i> Loading</button>
