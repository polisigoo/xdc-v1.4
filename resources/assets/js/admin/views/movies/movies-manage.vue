    ...
    <div>
        ...
        <div class="k1_manage_table">
            ...
            <div v-if="Object.keys(data).length  > 0 ">
                ...
                <div v-else>
                    <div class="table-responsive mt-2" v-if="Object.keys(data_search).length  > 0  && data_search.movies !== null ">
                        <div class="table table-hover">
                            <thead>
                                <th>Cover</th>
                                <th>Name</th>
                            ...
                            <tbody>
                                <tr v-for="(item, index) in data_search.movies" :key="index">
                                    ...
                                    <td v-if="item.m_cloud == 'aws' ">
                                        <img :src=" $config.getCloudfrontLink('sm', 'poster') + item.poster" :alt="item.name" width="40">
                                    </td>
                                    ...
                                    <td>
                                        <div class="btn-group" role="group">
                                            ...
                                            <button v-if="!button_loading" class="btn btn-sm btn-danger btn-sm mr-2" @click="DELETE(item.id,index)" :id="item.id">Delete
                                            </button>
                                            <button v-if="button_loading === item.id" class="btn btn-sm btn-danger btn-sm mr-2" disabled>
                                                <i id="btn-progress"></i> Loading
                                            </button>

                                            <button class="btn btn-sm btn-success btn-sm mr-2" v-if="!item.show" @click="AVAILABLE_IT(item.id, index)">Available
                                            </button>
                                            <button class="btn btn-sm btn-warning btn-sm mr-2" v-if="item.show" @click="AVAILABLE_IT(item.id, index)">Unavailable
                                            </button>
                                        </div>
                                    </td>
                                </tr>
                ...
                axios.post('/api/admin/update/movie/available', {
                    ...
                    if (response.status === 200) {
                        if (response.data.type === 'available') {
                            this.data.movies.data[index].show = true;
                        } else if (response.data.type === 'unavailable') {
                            this.data.movies.data[index].show = false;
                        }
                    }
                }, error => {
