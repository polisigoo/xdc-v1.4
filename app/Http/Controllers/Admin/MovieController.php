...
class MovieController extends Controller
{
    ...
    public function getMovieDetails($id)
    {
        $getMovie = DB::table('movies')
            ->selectRaw('movies.m_id AS id, movies.m_name AS name, movies.m_poster AS poster, movies.m_desc AS overview, movies.m_runtime AS runtime, movies.m_year AS year, movies.m_genre AS genre, movies.m_age AS age , movies.m_rate AS rate, movies.m_backdrop AS backdrop,movies.m_youtube AS youtube, movies.m_age AS age, movies.m_type AS type, movies.m_cloud AS cloud, videos.video_url')
    ...
    public function uploadMovieVideoToLocal($request)
    {
        if ($request->has('video_link')) {
            ...
            if ($resolution[0]['Container'] === 'ts') {
                ...
                $transcoding = $this->transcodingToHLS($path, $resolution, $addtype->m_name, $randomName, 'Video Convert To HLS Playlist Wait, its take time', $request->tmdb_id);
    ...
    public function uploadMovieVideoToAWS($request){
        if ($request->has('video_link')) {
            ...
            if ($resolution[0]['Container'] === 'ts') {
                ...
                $transcoding = $this->transcodingToHLS($path, $resolution, $addtype->m_name, $randomName, 'Video Convert To HLS, its take time', $request->tmdb_id);
                ...
                if ($transcoding) {
                    ...
                    $video->movie_id = $request->id;
                    $video->resolution = '720';
                    $video->video_url =  $addtype->m_name . '/' . $newNameM3U8;
                    $video->save();
            ...
            Storage::deleteDirectory('public/temp');
            Storage::deleteDirectory('public/videos/' . $addtype->m_name . '/');
