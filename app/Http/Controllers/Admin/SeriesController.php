...
class SeriesController extends Controller
{
    ...
    public function getSeries($id)
    {
        $getSeries = DB::table('series')
            ->selectRaw('series.t_id AS id, series.t_name AS name, series.t_poster AS poster, series.t_desc AS overview, series.t_year AS year, series.t_genre AS genre, series.t_rate AS rate, series.t_backdrop AS backdrop, series.t_age AS age, series.t_cloud AS cloud')
            ->where('t_id', $id)
            ->first();
    ...
    public function getEpisodeDetails($id)
    {
        ...
        $get = DB::table('episodes')
            ->select('episodes.id', 'episodes.name', 'episodes.overview', 'episodes.backdrop', 'episodes.episode_number', 'episodes.cloud', 'episodes.season_number', 'episodes.type', 'videos.video_url')
    ...
    public function updateEpisodeDetails(Request $request)
    {
        ...
        if ($check->cloud == 'aws') {
            ...
            // Embed Link
            $video = Video::where('episode_id', $request->id)->first();
            if ($check->type === 'embed') {
    ...
    public function uploadEpisodeVideoToLocal($request)
    {
        ...
        if ($request->video_link !== "empty") {
            ...
            if ($resolution[0]['Container'] === 'ts') {
                foreach ($listVideoNameAndId as $videoKey => $videoValue) {
                    ...
                    if ($videoValue['episode_name'] !== null) {
                        $path_upload = $request->series_name . '/' . $videoValue['episode_name'] . '_' . $videoValue['season_number'] . '_' . $videoValue['episode_number'] . '/';
                    } else {
                        $path_upload = $request->series_name . '/' . $videoValue['id'] . '_' . $videoValue['season_number'] . '_' . $videoValue['episode_number'] . '/';
                    }
                    ...
                    foreach ($resolution as $key => $value) {
                        ...
                        if ($value['Resolution'] === '4k') {
                            ...
                            $converVeryHigh = FFMpeg::fromDisk('public')
                                ...
                                ->save('videos/' . $path_upload . '/' . $newNameMP4);
        ...
        } elseif ($request->embed !== 'undefined' && !empty($request->embed)) {
        ...
        if ($request->video_link !== "empty") {
            ...
            if ($resolution[0]['Container'] === 'ts') {
                foreach ($listVideoNameAndId as $videoKey => $videoValue) {
                    ...
                    $transcode = $this->transcodingToHLS($videoValue['path'], $resolution, $videoValue['id'], $randomName, 'Episode ' . $videoValue['episode_number'] . ' is in processing', $request->tmdb_id);
                    ...
                    if ($transcode) {
                        ...
                        if ($videoValue['episode_name'] !== null) {
                            ...
                            $path_upload = $request->series_name . '/' . $request->series_name . '_' . $videoValue['season_number'] . '_' . $videoValue['episode_number'] . '/';
                        }
                        ...
                        $s3->uploadDirectory(storage_path('/app/public/videos/' . $videoValue['id'] . '/'), config('aws.private_bucket'), '/videos/' . $path_upload, []);
